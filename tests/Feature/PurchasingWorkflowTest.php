...
test('purchasing status transitions are explicit and terminal states are immutable', function () {
    expect(PurchaseRequestStatus::Draft->canTransitionTo(PurchaseRequestStatus::Submitted))->toBeTrue()
        ...
        ->and(ExpenseStatus::Approved->canTransitionTo(ExpenseStatus::Paid))->toBeTrue()
...
test('phase four permissions are seeded with purchase request canvass and purchase order tables', function () {
    ...
    foreach (['purchase_requests', 'purchase_request_lines', 'canvass_quotations', 'purchase_orders', 'purchase_order_lines', 'receiving_records', 'receiving_record_lines', 'supplier_invoices', 'supplier_invoice_lines', 'supplier_payments', 'supplier_payment_allocations', 'expenses'] as $table) {
        expect(Schema::hasTable($table))->toBeTrue();
    }

    foreach (['receivings'] as $table) {
