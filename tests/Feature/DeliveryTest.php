...
test('delivery lifecycle authorization print and prohibited effects are enforced', function () {
    ...
    expect($d->fresh()->status)->toBe(DeliveryStatus::Accepted)->and(InventoryMovement::count())->toBe(0)->and(Schema::hasTable('journal_entries'))->toBeFalse()->and(Schema::hasTable('sales_invoices'))->toBeTrue()->and(SalesInvoice::count())->toBe(0);
});
