...
test('quotations create no downstream financial inventory or order effects', function () {
    ...
    expect(Schema::hasTable('sales_orders'))->toBeTrue()->and(Schema::hasTable('sales_invoices'))->toBeTrue()->and(SalesInvoice::count())->toBe(0)->and(Schema::hasTable('customer_payments'))->toBeTrue()->and(CustomerPayment::count())->toBe(0)->and(InventoryMovement::count())->toBe(0)->and(JournalEntry::query()->count())->toBe(0);
});
