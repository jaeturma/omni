...
class UpdateProductServiceRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $productService instanceof ProductService && (bool) $this->user()?->can('update', $productService);
    }
    ...
    {
        return [
            'sku' => ['required', 'string', 'max:50', 'regex:/^[A-Z0-9_-]+$/', Rule::unique(ProductService::class)->ignore($this->route('product_service'))],
            'barcode' => ['nullable', 'string', 'max:100', Rule::unique(ProductService::class)->ignore($this->route('product_service'))],
